...
def no_failure(network_name, topo_dir, backup_links):
    ...
    BASE_SNAPSHOT_NAME = network_name + "whole_topo"
...
def one_link_fails(network_name, topo_dir, fail_link, backup_link):
    ...
    result = bf.q.reachability(pathConstraints=PathConstraints(startLocation = '/source/'), headers=HeaderConstraints(dstIps='r_w'), actions='SUCCESS').answer(LINKS_INACTIVE_SNAPSHOT_NAME).frame()
...
if __name__ == '__main__':
    network_name = 't1'
    topo_path = "./networks/t1/whole_topo"
    ...
    flows1, time1 = no_failure('t1', './networks/t1/whole_topo', backup_links)
    flows2, time2 = no_failure('t2', './networks/t2/whole_topo', backup_links2)
