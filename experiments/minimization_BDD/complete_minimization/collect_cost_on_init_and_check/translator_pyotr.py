...
def normalization():
    ...
    for i in tqdm(range(contrad_count)):
        row = cursor.fetchone()
        is_contrad, cond_time = iscontradiction(solver, row[1])
        cond_time_list.append(cond_time)
    ...
    for i in tqdm(range(redun_count)):
        row = cursor.fetchone()
        has_redun, result, redun_time = has_redundancy(solver, tauto_solver, row[1])
        redun_time_list.append(redun_time)
        if has_redun:
            if result != '{}':
    ...
    conn.commit()
    # redun_count=0
    return {"contrdiction":cond_time_list, "redundancy":redun_time_list}
...
def iscontradiction(solver, conditions):
    ...
    if len(conditions) == 0:
        return False, {"init": total_init, "checking":0}
    ...
    for c in conditions:
        ...
        end_init = time.time()
        total_init += end_init - begin_init
    ...
    if result == z3.unsat:
        solver.pop()
        return True, {"init": total_init, "checking":end_checking-begin_checking}
    else:
        solver.pop()
        return False, {"init": total_init, "checking":end_checking-begin_checking}
...
def has_redundancy(solver, tau_solver, conditions):
    ...
    if len(conditions) == 0:
        return has_redundant, result,  {"init": total_init, "checking":total_checking}
    ...
    if len(conditions) == 1:
        ...
        if tau_solver.check() == z3.sat:
            is_tauto = False
        end_checking = time.time()
        total_checking = end_checking - begin_checking
        tau_solver.pop()

        if is_tauto:
            return is_tauto, '{}', {"init": total_init, "checking":total_checking}
        else:
            return has_redundant, result,  {"init": total_init, "checking":total_checking}
    else:        
    ...
    if has_redundant:
        ...
        if is_tauto:
            return is_tauto, '{}',  {"init": total_init, "checking":total_checking}
        return has_redundant, final_result, {"init": total_init, "checking":total_checking}
