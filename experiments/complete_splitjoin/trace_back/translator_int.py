...
def normalization():
    ...
    for i in tqdm(range(contrad_count)):
        ...
        # print(", ".join(row[1]))
        check_or_cond = preprocessing(", ".join(row[1]))
        res = check_or_cond.count(pattern)
        total_num += res

        if is_contrad:
            del_tuple.append(row[0])
    print("\nNumber of repeated contradiction: ", total_num)
    if len(del_tuple) == 0:
        pass
...
def has_redundancy(solver, tau_solver, conditions):
    ...
    for idx, c in enumerate(conditions):
        expr = analyze(c)
        processed_conditions[idx] = eval(expr)
