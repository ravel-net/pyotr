...
typedef struct
{
	...
	char 	 c_var[10]; 	    /* TODO: Make this dynamic */
...
#define c_variables(inetptr) \
	(((inet_struct *) VARDATA_ANY(inetptr))->c_var)
 ...
 */
#define DatumGetInetPP(X)	((inet_faure *) PG_DETOAST_DATUM_PACKED(X))
#define InetPGetDatum(X)	PointerGetDatum(X)
#define PG_GETARG_INET_PP(n) DatumGetInetPP(PG_GETARG_DATUM(n))
#define PG_RETURN_INET_P(x) return InetPGetDatum(x)
/* obsolescent variants */
#define DatumGetInetP(X)	((inet_faure *) PG_DETOAST_DATUM(X))
