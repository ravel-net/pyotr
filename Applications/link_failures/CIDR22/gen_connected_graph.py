...
import math
import random
from rtable_fc import RtableFc
...
def tuple_to_nxdict(input):
    ...
    for i in input:
        ...
        output[i[0]][i[1]] = {'weight': 1}
    return output

"""
load instance spanning tree in fc table
"""
def load_tree_in_f(dbname, trees, dest):

    conn = psycopg2.connect(host=cfg.postgres["host"], database=dbname, user=cfg.postgres["user"], password=cfg.postgres["password"])

    cursor = conn.cursor()

    for k in tqdm(range(len(trees))): # each mst / each Fc
        fc_name = "fc_{}".format(k)

        sql = "DROP TABLE IF EXISTS {}".format(fc_name)
        cursor.execute(sql)

        sql = "create table {} (n1 integer, n2 integer, s integer[])".format(fc_name)
        cursor.execute(sql)
        conn.commit()

        edges = list(trees[k].edges())
        ...
        # initial edges
        '''
        stores links who contain dest node(root node) first
        inverse link if tail is dest
        '''
        ...
        sql = "insert into {} values(%s, %s, %s)".format(fc_name)
        cursor.executemany(sql, fc_graph)
        conn.commit()

"""
num: the number of instances required to be generated
"""
def add_filter(dbname, nodes, num):
    conn = psycopg2.connect(host=cfg.postgres["host"], database=dbname, user=cfg.postgres["user"], password=cfg.postgres["password"])
    ...
    for n in tqdm(range(num)):
        fc_name = "fc_{}".format(n)

        cursor.execute("update {} set s = array_append(s, n1) where n1 in {}".format(fc_name, tuple(picked_nodes)))


if __name__ == '__main__':
    as_numbers = [4755, 3356, 2914, 7018]
    roots = [108, 1711, 7121, 5675]
    nums = [1000, 1000, 50, 50]

    data_conn = psycopg2.connect(host=cfg.postgres["host"], database=cfg.postgres["db"], user=cfg.postgres["user"], password=cfg.postgres["password"])

    for c, as_num in enumerate(as_numbers):
        print("AS number: ", as_num)

        data_cur = data_conn.cursor()
        data_cur.execute("select * from topo_{} where n1 != n2;".format(as_num))
        edges= data_cur.fetchall()

        # transform
        edges = tuple_to_nxdict(edges)

        # determine the number of trees need to be generated
        num = nums[c]

        graph = nx.Graph(edges)

        # choose the subgraph which contains root
        for index, sub in enumerate(nx.connected_components(graph)):
            H = graph.subgraph(sub)
            if roots[c] in list(H.nodes):
                subgraph = H.copy()
                break
        
        graph_yamada = Yamada(subgraph, num)
        trees = graph_yamada.spanning_trees()

        db_name = 'cidr_fc_{}'.format(as_num)

        # load in f table
        print("Load tree in fc")
        load_tree_in_f(db_name, trees, roots[c])
        print("Done!")
        ...
        for n in tqdm(range(num)):
            fc_name = 'fc_{}'.format(n)
            r_gene = RtableFc(dbname=db_name, f_table=fc_name, as_number=as_num)

            r_gene.r1()
            count = r_gene.rn()
            r_gene.union(count=count)
            r_gene.select(dest=roots[c])
        print("Done.")
    ...
    # for n in tqdm(range(num)):
    #     fc_name = 'fc_{}'.format(n)
    #     r_gene = RtableFc(dbname=db_name,f_table=fc_name, as_number=0)

    #     r_gene.r1()
    #     count = r_gene.rn()
    #     r_gene.union(count=count)

    #     r_gene.select(dest=5)
    # print("Done.")
