...
if __name__ == "__main__":
    ...
    # Also note that the header conditions are represented in z3 format while the body conditions are represented in sql format. TODO: Write a Z3 to SQL parser
    p1 = "R(x,y)[And{y = 10, y < 20}] :- L(x,y,z)[y = 10 and y < 20], Q(z)\nR(x,y) :- L(x,q,z), Q(z)"
    program1 = program.DT_Program(p1, {"R":["integer", "int4_faure"], "L":["integer", "int4_faure", "int4_faure"], "Q":["int4_faure"]}, domains=['1', '2'], c_variables=['z','y'], reasoning_engine='z3', reasoning_type='Int', datatype='int4_faure', simplification_on=True, c_tables=["R", "L", "Q"])
    program1.minimize()
    if (str(program1) != "R(x,y)[And(y == 10,  y < 20)] :- L(x,y,z)[y == 10 and y < 20],Q(z)\nR(x,y) :- L(x,q,z),Q(z)"):
        print("Test 6 failed")
        exit()
    else:
        print("Test 6 passed")
