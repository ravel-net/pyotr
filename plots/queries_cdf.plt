...
set output "queries_cdf.pdf"

plot 'queries_cdf.dat' using ($2):($1) title '𝜎_{inet}' with p linestyle 8,\
'queries_cdf.dat' using ($3):($1) title '𝜎_{typed-inet}' with p linestyle 3,\
'queries_cdf.dat' using ($4):($1) title '𝜎_{text}' with p linestyle 2,\
'queries_cdf.dat' using ($5):($1) title '⋈_{inet}' with p linestyle 4,\
'queries_cdf.dat' using ($6):($1) title '⋈_{typed-inet}' with p linestyle 5,\
'queries_cdf.dat' using ($7):($1) title '⋈_{text}' with p linestyle 6 
